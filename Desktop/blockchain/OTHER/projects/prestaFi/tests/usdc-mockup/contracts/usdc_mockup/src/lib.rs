...
impl UsdcMockup 
{
    ...
    {
        let actual_value: u32 = env.storage().instance().get(&COUNTER).unwrap_or(0);

        actual_value
    }

    pub fn get_future_value(env: Env) -> u32
    {
        let mut future_value: u32 = env.storage().instance().get(&COUNTER).unwrap_or(0);
        future_value += 1;

        future_value
    }

    pub fn get_map_value(env: Env, user_address: Address) -> i128
    {
        env.storage()
            .instance()
            .get(&DataKey::ShareBalances(user_address))
            .unwrap_or(0)
    }

    pub fn set_map_value(env: Env, user_address: Address, amount: i128)
    {
            env.storage()
                .instance()
                .set(&DataKey::ShareBalances(user_address), &amount);
    }
